...
// --------------------------------------------------------------------------------------------------------------------
// PATCH MultiPhiMo (round 3.2b, 2026-09-23, NON upstream DPF -- da riapplicare a mano ad
// ogni aggiornamento di questo clone vendorizzato di DPF, vedi claude/vst3_native_stato.md):
// di serie, quando DISTRHO_PLUGIN_WANT_MIDI_INPUT e' abilitato, DPF espone qui 130 CC x 16
// canali = 2080 pseudo-parametri "MIDI CC" -- marcati hidden (V3_PARAM_IS_HIDDEN in
// getParameterInfo, DistrhoPluginVST3.cpp) ma Reaper li mostra comunque, tutti, nella
// finestra di automazione insieme ai 37 parametri veri (bug noto, mai risolto upstream:
// github.com/DISTRHO/DPF/issues/345). MultiPhiMo usa solo 37 CC fissi, tutti su canale
// MIDI 1 (run() ignora comunque il canale per il parsing MIDI reale via `status & 0xF0`,
// vedi PluginMultiPhiMo.cpp -- limitare l'automazione-parametro-VST3 al canale 1 non
// toglie nulla al MIDI hardware reale su altri canali). La costante sotto sostituisce
// "130 * 16"; la tabella completa (numero CC + nome) vive in DistrhoPluginVST3.cpp
...
enum Vst3InternalParameters {
    ...
    kVst3InternalParameterCount = kVst3InternalParameterMidiCC_start + kMultiPhiMoUsedMidiCCCount
   #else
