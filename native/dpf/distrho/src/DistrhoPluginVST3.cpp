...
// --------------------------------------------------------------------------------------------------------------------
// PATCH MultiPhiMo (round 3.2b, 2026-09-23, NON upstream DPF -- vedi il commento gemello
// su kMultiPhiMoUsedMidiCCCount in DistrhoPluginVST.hpp per il perche'). Tabella completa
// dei 38 MIDI CC realmente usati da MultiPhiMo (numero + stesso "symbol" gia' mostrato per
// il parametro host corrispondente, per correlazione immediata in Reaper), tutti su canale
// MIDI 1 -- vedi PluginMultiPhiMo.cpp per il dispatch CC reale (identico, verificato contro
// il sorgente prima di scrivere questa tabella). Sostituisce la numerazione generica DPF
// channel*130+cc (2080 pseudo-parametri) usata da get_midi_controller_assignment/
// getParameterInfo/appendCC piu' sotto.
#if DISTRHO_PLUGIN_WANT_MIDI_INPUT
struct MidiCCMapEntry { uint8_t cc; const char* symbol; };

// Rimappata 2026-09-25 (decisa con l'utente): nessun CC standard riassegnato tranne CC7=volume.
static constexpr MidiCCMapEntry kMultiPhiMoMidiCCTable[] = {
    { 7,  "gain" },
    { 16, "mode" }, { 17, "exciter_select" }, { 18, "resonator_select" },
    { 20, "exciter_param_1" }, { 21, "exciter_param_2" }, { 22, "exciter_param_3" },
    { 23, "exciter_param_4" }, { 24, "exciter_param_5" }, { 25, "exciter_param_6" },
    { 26, "exciter_param_7" }, { 27, "exciter_param_8" }, { 28, "exciter_param_9" },
    { 80, "resonator_param_1" }, { 81, "resonator_param_2" }, { 82, "resonator_param_3" },
    { 83, "resonator_param_4" }, { 85, "resonator_param_5" }, { 86, "resonator_param_6" },
    { 87, "resonator_param_7" }, { 88, "resonator_param_8" }, { 89, "resonator_param_9" },
    { 90, "resonator_param_10" },
    { 102, "target_spectral_centroid" }, { 103, "target_spectral_spread" },
    { 104, "target_spectral_rolloff" },  { 105, "target_spectral_flatness" },
    { 106, "target_roughness" },         { 107, "target_harmonic_tension" },
    { 108, "target_inharmonicity" },     { 109, "target_formant_f1" },
    { 110, "target_formant_f2" },        { 111, "target_formant_f3" },
    { 112, "target_mod_rate" },          { 113, "target_mod_depth" },
...
static_assert(sizeof(kMultiPhiMoMidiCCTable) / sizeof(kMultiPhiMoMidiCCTable[0]) == kMultiPhiMoUsedMidiCCCount,
              "kMultiPhiMoMidiCCTable size must match kMultiPhiMoUsedMidiCCCount (DistrhoPluginVST.hpp)");

// -1 se cc non e' tra quelli usati da MultiPhiMo.
static inline int32_t multiPhiMoCcToCompactIndex(const uint8_t cc) noexcept
{
    for (uint32_t i = 0; i < kMultiPhiMoUsedMidiCCCount; ++i)
    {
        if (kMultiPhiMoMidiCCTable[i].cc == cc)
...
class PluginVst3
{
    ...
    struct InputEventList {
        ...
        bool appendCC(const int32_t sampleOffset, v3_param_id paramId, const double normalized) noexcept
        {
            ...
            // PATCH MultiPhiMo (round 3.2b): paramId e' ora un indice compatto nella tabella
            // kMultiPhiMoMidiCCTable (38 CC reali, canale MIDI 1 fisso) invece del vecchio
            // channel*130+cc generico DPF -- vedi tabella/commento sopra. Aftertouch/pitchbend
            // (cc 128/129 nello schema originale) non esistono in questa tabella, MultiPhiMo
            // non li usa.
            if (paramId >= kMultiPhiMoUsedMidiCCCount)
                return false;

            eventStorage.type = CC_Normal;
            eventStorage.midi[0] = 0; // canale MIDI 1 (0-based) -- vedi nota tabella sopra
            eventStorage.midi[1] = kMultiPhiMoMidiCCTable[paramId].cc;
...
public:
    ...
    v3_result process(v3_process_data* const data)
    {
        ...
        // PATCH MultiPhiMo (2026-09-25, NON upstream DPF): DPF non scriveva mai
        // channel_silence_bitset delle uscite (obbligo del plugin nello standard VST3); l'host
        // poteva quindi leggere un flag "uscita silenziosa" rimasto dal chiamante e tenere il
        // proprio buffer (= audio in ingresso). Qui: uscite dichiarate NON silenziose.
        {
            for (int32_t b = 0; data->outputs != nullptr && b < data->num_output_buses; ++b)
                data->outputs[b].channel_silence_bitset = 0;
        }
    ...
    v3_result getParameterInfo(const int32_t rindex, v3_param_info* const info) const noexcept
    {
        ...
        if (rindex < kVst3InternalParameterCount)
        {
            // PATCH MultiPhiMo (round 3.2b): index e' ora nella tabella compatta
            // kMultiPhiMoMidiCCTable (38 CC reali, canale 1) invece dei 2080 generici
            // "MIDI Ch.N CC.M" di DPF -- titolo con lo stesso "symbol" gia' mostrato per
            // il parametro host corrispondente, per correlazione immediata in Reaper.
            ...
            info->step_count = 127;
            if (index < kMultiPhiMoUsedMidiCCCount)
            {
                char ccstr[40];
                snprintf(ccstr, sizeof(ccstr), "CC%d %s", kMultiPhiMoMidiCCTable[index].cc, kMultiPhiMoMidiCCTable[index].symbol);
                strncpy_utf16(info->title, ccstr, 128);
                snprintf(ccstr, sizeof(ccstr), "CC%d", kMultiPhiMoMidiCCTable[index].cc);
                strncpy_utf16(info->short_title, ccstr, 128);
            }
            return V3_OK;
        }
       #endif
...
struct dpf_midi_mapping : v3_midi_mapping_cpp {
    ...
    static v3_result V3_API get_midi_controller_assignment(void*, const int32_t bus, const int16_t channel, const int16_t cc, v3_param_id* const id)
    {
        ...
        // PATCH MultiPhiMo (round 3.2b): solo canale MIDI 1 e i 38 CC realmente usati --
        // vedi kMultiPhiMoMidiCCTable/commento sopra (era: qualunque canale 0-15, cc 0-129,
        // 2080 combinazioni sempre offerte come automatabili).
        if (channel != 0)
            return V3_FALSE;

        const int32_t index = multiPhiMoCcToCompactIndex(static_cast<uint8_t>(cc));
        if (index < 0)
            return V3_FALSE;

        *id = kVst3InternalParameterMidiCC_start + static_cast<uint32_t>(index);
        return V3_TRUE;
    }
